...
class GaussianBayesianBandit(BetaAlgo):
  def __init__(self, bandit):
    super().__init__(bandit)

  @staticmethod
  def name():
    return 'gaussian bayesian'
  
  def get_action(self):
    """ Gaussian parameters mean and variance are sampled from the beta"""
    theta = np.random.norm(self.alpha, self.beta)
    return theta.argmax()
